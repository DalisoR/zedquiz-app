...
function AddQuestionsPage({ selectedQuiz, setPage }) {
  ...
  const handleAddQuestion = async event => {
    ...
    if (imageFile) {
      ...
      const fileName = `${selectedQuiz.id}/${Date.now()}-${Math.random()
        .toString(36)
        .slice(2)}.${fileExt}`;
      ...
      if (uploadError) {
        ...
        if (msg === 'Failed to fetch') {
          alert(
            'Error uploading image: Network/CORS error. Ensure the storage bucket "question_images" exists and that you are online.'
          );
        } else {
  ...
  return (
    <div className='main-container'>
      <header className='main-header admin-header'>
        ...
        <button className='back-button' onClick={() => setPage('admin-dashboard')}>
          Finish
        </button>
      </header>
