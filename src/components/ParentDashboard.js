    ...
    const handleLinkChild = async (e) => {
        ...
        try {
            ...
            if (linkError) {
                ...
                setChildEmail('');
                // Refresh list
                const { data: refreshedData } = await supabase.from('parent_child_relationships').select('id, status, child:profiles!child_id ( id, full_name, email )').eq('parent_id', user.id);
