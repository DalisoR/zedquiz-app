            ...
            <div className="content-body">
                            ...
                            {pendingRequests.map(req => (
                                <li key={req.id}>
                                    ...
                                    <div className="booking-actions">
                                        <button className="confirm-btn" onClick={() => handleDecision(req.id, 'confirm')}>Confirm</button>
                                        <button className="reject-btn" onClick={() => handleDecision(req.id, 'reject')}>Reject</button>
                                    </div>
                                </li>
                            ))}
                        </ul>
                ...
                <div className="card">
                    <h3>Upcoming Lessons ({confirmedLessons.length})</h3>
                     {loading ? <p>Loading...</p> : confirmedLessons.length > 0 ? (
                        <ul className="booking-list">
