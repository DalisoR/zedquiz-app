...
function UsageLimitGuard({
  ...
  const { canPerformAction, getLimitMessage, getUpgradeMessage, checkLimit, loading } =
  ...
  return (
    ...
    >
      ...
      <h3 style={{ margin: '0 0 1rem 0', color: '#ef4444' }}>Daily Limit Reached</h3>

      <p style={{ margin: '0 0 1rem 0', color: '#666' }}>{limitMessage}</p>
      ...
      {upgradeMessage && showUpgrade && (
        ...
        >
          <p style={{ margin: '0 0 1rem 0', color: '#1e40af' }}>💡 {upgradeMessage}</p>
...
export function UsageProgressBar({
  ...
  if (limitInfo.isUnlimited) {
    return showLabel ? (
      <div style={{ fontSize: '0.875rem', color: '#10b981', fontWeight: 600 }}>✨ Unlimited</div>
    ) : null;
  }
...
export function UsageSummaryWidget({ currentUser, setPage }) {
  ...
  if (loading) {
    return (
      <div className='card'>
        <div style={{ textAlign: 'center', padding: '1rem' }}>Loading usage data...</div>
      </div>
    );
  }
