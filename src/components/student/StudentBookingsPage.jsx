...
import styles from './StudentBookingsPage.module.css';
...
function StudentBookingsPage({ currentUser, setPage, setSelectedBooking }) {
  ...
  const upcomingLessons = bookings.filter(
    b => b.status === 'confirmed' && new Date(b.requested_datetime) > new Date()
  );
                ...
                <li key={lesson.id}>
                  ...
                  <div className='booking-actions'>
                    {lesson.reviews.length === 0 ? (
                      <button className='review-btn' onClick={() => handleLeaveReview(lesson)}>
                        Leave a Review
                      </button>
                    ) : (
...
StudentBookingsPage.defaultProps = {
  // Add default props
};
